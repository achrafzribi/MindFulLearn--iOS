...
class Webservice {
    var urlfinal = "http://172.18.6.106:3000/api/"
    func getPosts(completion: @escaping ([Post]?) -> ()) {
        guard let url = URL(string: urlfinal+"user/add") else {
    ...
    func loginUser(email: String, password: String, completion: @escaping (UserData) -> ()) {
        ...
        let apiUrl = urlfinal+"user/login"
        ...
        func showUser(completion: @escaping (UserData?) -> ()) {
             ...
             */
            AF.request(urlfinal+"user/find/6556d25c7642485ca411f94c", method: .get)
                .validate()
                .responseData { response in
                    switch response.result {
                    case .success(let data):
        ...
        func deleteUser(completion: @escaping (Bool) -> ()) {
            ...
            guard let url = URL(string: urlfinal+"user/:id") else {
                completion(false)
                return
            }
    ...
    func updateUser(user: UserData, completion: @escaping (Bool) -> ()) {
         let url = URL(string: urlfinal+"user/update/\(user.__id!)")
    ...
    func ChangePassword(email: String, password: String, completion: @escaping (String) -> ()) {
        // Replace the URL with your actual login endpoint
        let apiUrl = urlfinal+"user/password"
       print(email)
        let parameters: [String: Any] = ["email": email, "password": password]
