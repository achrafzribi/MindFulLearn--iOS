...
extension Message: Codable {
    init(from decoder: Decoder) throws {
        ...
        case String(describing: Int.self): // Handle the case for connected clients count
                    self = .connectedClients(try container.decode(Int.self, forKey: .payload))
    ...
    func encode(to encoder: Encoder) throws {
        ...
        case .connectedClients(let count): // Encode the count for connected clients
                    try container.encode(count, forKey: .payload)
                    try container.encode(String(describing: Int.self), forKey: .type)
        }
    }
